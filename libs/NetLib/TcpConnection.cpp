...
void TcpConnection::send(const string & message)
{
    if (state_ == kConnected)
    {
        ...
        {
            auto cb = std::bind(static_cast<void (TcpConnection::*)(const string&)>(&TcpConnection::sendInLoop),
                this,     // FIXME
                message);
            loop_->runInLoop(cb);
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
  ...
  //      std::stringstream ss;
        ...
        else // nwrote < 0
        {
            ...
            if (errorCode == WSAEWOULDBLOCK && (time(NULL) - startTime) > m_nTimeOut)
            {
                msg = "超时";
                faultError = true;
                m_errorCallbackCB(msg);
            }
