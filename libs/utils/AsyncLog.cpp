...
LOG_LEVEL_ASYNC CAsyncLog::m_nCurrentLevel = LOG_LEVEL_DEBUG_ASYNC;
...
void CAsyncLog::setLevel(LOG_LEVEL_ASYNC nLevel)
{
    if (nLevel < LOG_LEVEL_TRACE_ASYNC || nLevel > LOG_LEVEL_FATAL_ASYNC)
        return;
...
{
    if (nLevel != LOG_LEVEL_CRITICAL_ASYNC)
    {
    ...
    if (nLevel != LOG_LEVEL_FATAL_ASYNC)
    {
...
bool CAsyncLog::output(long nLevel, const char* pszFileName, int nLineNo, const char* pszFmt, ...)
{
    ...
    std::string strFileLog(pszFileName);
    int nPos = strFileLog.find_last_of("\\");

    std::string strTemp = std::string(" ").append(strFileLog.substr(nPos + 1).append(":").append(std::to_string(nLineNo)).append("  "));
    //strLine += szFileName;
    strLine += strTemp;
