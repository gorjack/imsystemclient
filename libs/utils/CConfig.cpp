...
void CConfig::setUserName(const QString &strName)
{
    ...
    QString strPath = PATH::getUserRootDir();
    QDir d(strPath);
    if (!d.exists())
    {
        d.mkpath(".");
...
{
    QString strConfigPath = PATH::getUserRootDir() + QDir::separator() + g_cAddrName;
    if (!loadFromFile(strConfigPath))
    {
        strConfigPath = PATH::getConfigDir() + QDir::separator() + g_cAddrName;
