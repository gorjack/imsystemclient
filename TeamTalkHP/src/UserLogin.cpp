...
UserLogin::~UserLogin()
{
	// 断开与 CUserClientCenter 的信号连接，防止对象删除后仍收到信号导致崩溃
	if (CUserClientCenter::instance())
	{
		disconnect(CUserClientCenter::instance(), SIGNAL(sigStatus(int, QString)), this, SLOT(onRegistStatus(int, QString)));
		disconnect(CUserClientCenter::instance(), SIGNAL(sigLogindStatus(UserLoginStatus, QString)), this, SLOT(onLogindStatus(UserLoginStatus, QString)));
	}
}
